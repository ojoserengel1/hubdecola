-- ============================================================================
-- Script: Aplicar Sistema de Chat
-- Data: 2024-12-06
-- Descrição: Cria sistema completo de chat entre Cliente e Admin
-- ============================================================================
-- 
-- INSTRUÇÕES:
...
DROP TRIGGER IF EXISTS trigger_create_chat_conversation ON profiles;
...
DROP POLICY IF EXISTS "Clients can view own conversation" ON chat_conversations;
DROP POLICY IF EXISTS "Admins can view all conversations" ON chat_conversations;
DROP POLICY IF EXISTS "Clients can view own messages" ON chat_messages;
DROP POLICY IF EXISTS "Admins can view all messages" ON chat_messages;
DROP POLICY IF EXISTS "Clients can insert own messages" ON chat_messages;
DROP POLICY IF EXISTS "Admins can insert messages" ON chat_messages;
DROP POLICY IF EXISTS "Clients can update own messages" ON chat_messages;
...
-- Criar conversas para clientes existentes
INSERT INTO chat_conversations (user_id)
...
SELECT 
    '✅ Sistema de chat criado com sucesso!' as status,
